...
def on_start(update, context):
	...
	update.message.reply_text("Let's create a new bet. Send the question.", \
...
def on_bet_question(update, context):
	...
	update.message.reply_markdown("Creating a new bet: *%s*." \
		% bet_question, reply_markup=ReplyKeyboardRemove())
...
def on_loop(update, context):
	...
	logger.info("User %s added a new variant %s to a bet: %s.", \
		user.first_name, variant, bets.table[bet_hash].question)
	...
	update.message.reply_text('Any more variants? Send /done to stop \
		me asking.', reply_markup=ReplyKeyboardRemove())
	return ANSWER_LOOP
...
def on_end_loop(update, context):
	...
	logger.info("User %s finished adding new variants to a bet: %s.", \
		user.first_name, bets.table[bet_hash].question)

	update.message.reply_text('Postpone for weeks, years, days, hours, \
		seconds, minutes, months. For example: in 1 week. Or backwards: \
		in -3 hours.', reply_markup=ReplyKeyboardRemove())
...
def on_deadline_shift(update, context):
	num_param_regex = \
		re.compile(r'(-?[0-9]+) (day|week|year|hour|minute|month|second)s?')
	...
	update.message.reply_text('Any more postpones? Send /done to stop me asking.', \
		reply_markup=ReplyKeyboardRemove())
	return DEADLINE_SHIFT
...
def on_end_deadline_shift(update, context):
	...
	update.message.reply_text('The deadline was succesfully set up to %s. \
		\nToday is %s.' % (bet.deadline.format('DD:MM:YYYY HH:mm ZZ'), \
			bet.start.format('DD:MM:YYYY HH:mm ZZ')))
